...
output "wp_service_url" {
  value = "${kubernetes_service.wpService.load_balancer.status.0.load_balancer.0.ingress.0.ip}"
...
resource "null_resource" "open_wp" {
  provisioner "local-exec" {
    command = "start chrome ${kubernetes_service.wpService.load_balancer.status.0.load_balancer.0.ingress.0.ip}" 
  }
